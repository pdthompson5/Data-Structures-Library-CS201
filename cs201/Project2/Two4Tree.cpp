...
class Two4Tree{
    ...
    */
    ...
    private:
        ...
        //can assume that it is a 2 node 
        Node* merge(Node* current){
            ...
            //determine child index of current
            int j;
            for(j = 0; j < parent->numChildren; j++){
            ...
            if(j < parent->numChildren-1){
                rightSibling = parent->children[parent->numChildren-1];
            }
            else{
            ...
            //case: sibling on either side is greater than a two node
            //right sibling is the big sibling 
            if(rightSibling != nullptr){
                if(rightSibling->size > 1){
                    current->addPair(parent->removePair(j));
                    parent->addPair(rightSibling->removePair(0)); 
                    current->addChild(rightSibling->removeChild(0));
                    return current;
                }
            }
            
            //left sibling is the big sibling 
            if(leftSibling != nullptr){
                if(leftSibling->size > 1){
                    current->addPair(parent->removePair(j-1));
                    parent->addPair(leftSibling->removePair(leftSibling->size - 1));
                    current->addChild(leftSibling->removeChild(leftSibling->numChildren - 1));
                    return current;
                }
            }
            

            //case: parent and sibling are two nodes (only root)
            if(parent->size == 1){
                if(leftSibling != nullptr && leftSibling->size == 1){
                    for(int k = 0; k < parent->numChildren; k++){
                        parent->removeChild(0);
                    }
                    for(int k = 0; k < current->numChildren; k++){
                        parent->addChild(current->removeChild(0));
                    }

                    for(int k = 0; k < leftSibling->numChildren; k++){
                        parent->addChild(leftSibling->removeChild(0));
                    }

                    parent->addPair(current->removePair(0));
                    parent->addPair(leftSibling->removePair(0));
                    delete current;
                    delete leftSibling;
                    return parent;
                }

                if(rightSibling != nullptr && rightSibling->size == 1){
                    for(int k = 0; k < parent->numChildren; k++){
                        parent->removeChild(0);
                    }
                    for(int k = 0; k < current->numChildren; k++){
                    ...
                    for(int k = 0; k < rightSibling->numChildren; k++){
                        parent->addChild(rightSibling->removeChild(0));
                    }
            ...
            //case: parent is a three or four node and siblings are two nodes
            if(rightSibling != nullptr){ 
                for(int k = 0; k < rightSibling->numChildren; k++){
                    current->addChild(rightSibling->removeChild(0));
                }

                parent->removeChild(j+1);
                current->addPair(parent->removePair(j));
                current->addPair(rightSibling->removePair(0));
                ...
                for(int k = 0; k < leftSibling->numChildren; k++){
                    current->addChild(leftSibling->removeChild(0));
                }
                
                parent->removeChild(j-1);
                current->addPair(parent->removePair(j-1));
                current->addPair(leftSibling->removePair(0));
        ...
        Node* findPred(Node* start, keytype k){
            Node* current = start;
            //find index
            int i;
        ...
        Node* split(Node* current, keytype k){
            ...
            leftNode->addChild(current->removeChild(0));
            rightNode->addChild(current->removeChild(0));
            rightNode->addChild(current->removeChild(0));
    ...
    public:
        ...
        */
        ...
        /*Runtime: O(lgn)
            ...
            cases: Encounter a 2 node
            Resolutions 
            if root 
                check if both children are 2 nodes
                    if so 
                    merge down and shrink tree
                ...
                check if both children are 2 nodes
                    ...
                    look at right sibiling
                    if 

                    check if right sibling has an extra pair to spare
                        if so then rotate 
                        else
                        merge down 


            once found: if it is an internal node
                find successor
                delete sucessor 
                delete inital pair
                add sucessor 
        ...
        */
        int remove(keytype k){
            ...
            search = search->determinePath(k);
            //itterates until the node containing the key is found (or returns 0 if not found)
            while(search->searchNode(k) == nullptr){
                

                //if a two node is found then preemptive merge
                if(search->size == 1){
                    search = merge(search);
                }
            ...
            search->updateParentKeysBelow(search, -1);
            //case: search is not a leaf node (termination case)
            if(search->numChildren > 0){
                if(search->size == 1){
                    search = merge(search);
                }
                
                //find the predecessor pair
                Node* pred = this->findPred(search, k);
                
                

                //wierd case where fixing the subtree causes the deletion element to move nodes
                //since this only happens when it is merged down, we can assume that it's new node is not a 2 node 
                if(search->searchNode(k) == nullptr){
                    search = this->nodeSearch(root, k);
                    int i;
                    for(i = 0; i < search->size; i++){
                        if(search->vals[i]->key == k){
                            break;
                        }
                    }
                    search->removePair(i);
                ...
                //remove the search pair from the node 
                for(int j = 0; j < search->size; j++){
                    if(search->vals[j]->key == k){
                        Pair* temp = search->removePair(j);
                        delete temp;
                        break;
                    }
                }

                //add the pred pair to the node 
                search->addPair(pred->removePair(pred->size-1));

                return 1;

            }

            //case: search is a two node leaf
            if(search->size == 1){
                merge(search);
            }


            //determines the index of the pair to remove 
            int i;
            for(i = 0; i < search->size; i++){
                if(search->vals[i]->key == k){
                    break;
...
// int main(){
//     Two4Tree<int, int>* test = new Two4Tree<int, int>();
//     for(int i = 0; i < 10; i++){
//         test->insert(i,i);
//     }
//     test->printTree();
